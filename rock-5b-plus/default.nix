...
in
{
  options.hardware.rock-5b-plus = {
    ...
    image = {
      useRepart = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = "Use systemd-repart to generate the disk image, and also set the appropriate 'filesystems' options for auto mounting, as well as systemd-boot and therefore UEFI boot flow";
      };
      ...
      generateImage = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = "Generate a disk image that boots using a sensible default UEFI boot flow, and place it in config.system.build.image";
      };
...
#  ];
  config = lib.mkIf cfg.enable (lib.mkMerge [
    ...
    (lib.mkIf (cfg.image.generateImage && cfg.image.useRepart) {
